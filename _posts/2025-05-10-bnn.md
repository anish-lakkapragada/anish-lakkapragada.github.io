...
tags:
  - ml, variational-inference, bayesian-neural-networks
categories:
  - ml
---
...
= \frac{p(D \mid \theta) p(\theta)}{\displaystyle \int_{\theta'} p(D \mid \theta') p(\theta') d\theta'}
$$
...
$$
\mathrm{KL}\bigl(q_{\phi}(\theta)\,\|\,p(\theta \mid D)\bigr)
= \mathbb{E}\!\bigl[\log \tfrac{q_{\phi}(\theta)}{p(\theta \mid D)}\bigr]
...
Now, given this fully‐trained approximate stand‐in distribution $q_{\phi}(\theta)$ for the true posterior $p(\theta \mid D)$, we can compute predictive distributions using \eqref{pdistribution}. Instead of tackling that integral directly, we’ll use Monte Carlo integration: sampling i.i.d. parameters $q_{\phi}(\theta)$ and averaging their predictions.
...
where $$\mathbb{E}_{-j}$$ means that we assume $$ \forall i \neq j, \beta*i \sim q*{\phi*i}(\beta_i)$$. Comparing this condition and our previous ELBO derivation, we can see that it does sort of set $q*{\phi_j}(\beta_j)$ in a way that optimizes an ELBO adjusted for single coordinate optimization. For a formal treatment of how this maximizes the ELBO, see these [lecture notes](https://www.cs.princeton.edu/courses/archive/fall11/cos597C/lectures/variational-inference-i.pdf). We now apply this CAVI algorithm to our defined problem by finding how we should update our parameter sets $\phi_0 = (m_0, s_0^2)$ and $\phi_1 = (m_1, s_1^2)$ at each iteration to satisfy our aforementioned condition. Before doing so, it will be helpful to compute $$ \log p(\beta, y)$$, or equivalently $$ \log p(\beta, y \mid x) $$ as we are treating $$\mathbf{x}$$ as fixed:
...
$$ \log p(\beta, y | X) = -\frac{1}{2\tau*0^2} \beta_0^2 - \frac{1}{2\tau_1^2} \beta_1^2 - \frac{1}{2\sigma^2} \sum*{i = 1}^n (y_i - \beta_0 - \beta_1 x_i)^2 + \text{const}$$
...
[^mc-note]: Namely, one would sample $K$ samples $$\beta_0, \dots, \beta_K \overset{\text{i.i.d}}{\sim} \beta \mid D$$ and then approximate $$ p\bigl(\hat y \mid D, x\bigr) $$ with $$ p\bigl(\hat y \mid D, x\bigr) = \int p\bigl(\hat y \mid \beta, x)\,p\bigl(\beta \mid D)\,\mathrm{d}\beta \approx \frac{1}{K} \sum\_{i = 1}^K p(\hat{y} \mid \beta_i, x) $$.
...
I have put all the code used for this blog post on this [GitHub repo](https://github.com/anish-lakkapragada/bnn-predictive-distributions). Most of the code shown in this blog post is located in `blr.py` and `cavi.py`.
