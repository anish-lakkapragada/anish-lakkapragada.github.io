...
$$
0 = \mathbb{E}[X_{t + s} - X_t \mid \mathcal{F}_t] = \mathbb{E}[\int_{t}^{t + s} a(X_s, s) dt \mid \mathcal{F}_t] + \mathbb{E}[\int_{t}^{t + s} b(X_s, s) dW_s \mid \mathcal{F}_t]
$$
...
So we have that $$0 = \mathbb{E}[\int_{t}^{t + s} a(X_s, s) dt \mid \mathcal{F}_t]$$. Brushing aside some technicalities, this also gives us $$0 = \int_{t}^{t + s} \mathbb{E}[a(X_s, s) \mid \mathcal{F}_t] dt$$. From here, we can cleverly use the first Fundamental Theorem of Calculus to get rid of the expectation: 

$$
\underset{s \to 0}{\lim} \frac{1}{s} \int_{t}^{t + s} \mathbb{E}[a(X_s, s) dt \mid \mathcal{F}_t] = \mathbb{E}[a(X_t, t) \mid \mathcal{F}_t] dt = a(X_t, t)
$$

But as we know $$\int_{t}^{t + s} \mathbb{E}[a(X_s, s) \mid \mathcal{F}_t] dt = 0$$ and so the above limit is equal to 0 $$\implies a(X_t, t) = 0$$. Furthermore $$t$$ was arbitrary and so $$\forall t, a(X_t, t) = 0 \implies X_t$$ is driftless.

Note that proving the opposite direction is simpler, at least intuitively: 

$$\mathbb{E}[X_{t + s} \mid \mathcal{F}_t] = \mathbb{E}[X_t + \int_{t}^{t + s} a(X_s, s) \ dt + \int_{t}^{t + s} b(X_s, s) dW_s \mid \mathcal{F}_t] = X_t + \mathbb{E}[\int_{t}^{t + s} b(X_s, s) dW_s \mid \mathcal{F}_t]= X_t$$ 
